function [aPlane]=fCreateTable(aOrigin,aAngle, aDist)

% Calcula os vetores diretores em função do ângulo dado
% Em X
aVetX=[1 tan(aAngle*pi()/180) tan(aAngle*pi()/180)];
aCosDirX=aVetX/norm(aVetX);

% Em Y
aVetY=[tan(aAngle*pi()/180) 1 tan(aAngle*pi()/180)];
aCosDirY=aVetY/norm(aVetY);

% Em Z
aVetZ=[tan(aAngle*pi()/180) tan(aAngle*pi()/180) 1];
